...
disease_specialists = {
    ...
    "hypothyroidism" : "Endrocronologist",
    "arthritis" : "Rheumatologist",
    "hypertension" : "Cardiologist"
}

doctors = {
    "General Physician" : "Dr.Sham Sunder",
    "Infectious Disease Specialist": "Dr.Anitha",
    "Pulmonologist" : " Dr.Prasanna Kumar Thomas",
    "Dermatologist" : " Dr.Pankaj Singhal",
    "Hepatologist" : " Dr.Astha Sharma",
    "Cardiologist" : "Dr.Anil Bhan ",
    "Immunologist" : "Dr.Manjeeta Das ",
    "Neurologist" : "Dr.Puneet Agarwal ",
    "Endrocronologist" : "Dr.Monika Sharma ",
    "Rheumatologist" : "Dr.Ramakrishnan "
 }
def doctor(dis):
    sp = disease_specialists.get(dis.lower(), "Specialist not found")
    print(sp)
    print(doctors.get(sp))
    return doctors.get(sp,"Doctors near you")
def specialist(dis):
        return disease_specialists.get(dis.lower(), "Specialist not found")
...
def get_bot_response():
    ...
    if "step" in session:
        if session["step"] == "Q_C":
            ...
            session.clear()
            if s.lower() == "quit":
                "Thank you for using our <b>DiagnoBotIQ</b>  " + name
            else:
                session["step"] = "FS"
                session["name"] = name
                session["age"] = age
    ...
    if session['step'] == "Depart":
        session['step'] = "BFS"
        return "Good to see you <b>" + session[
            "name"] + "</b>, now I will be asking some few questions about your symptoms to see what you should do. Type <b>Start</b> to start diagnostic!"
    if session['step'] == "BFS":
        session['step'] = "FS"  # first symp
        return "Can you precise your main symptom <b>" + session["name"] + "</b> ?"
    if session['step'] == "FS":
        sym1 = s
    ...
    if session['step'] == "DIS":
        ...
        if len(session["symv"]) > 0:
            if symts[0] not in session["all"] and symts[0] not in session["asked"]:
                ...
                session["asked"] = asked
                symts = session["symv"]
                msg = "Are you experiencing <b>" + clean_symp(symts[0]) + "</b> ?"
                return msg
            else:
                del symts[0]
                session["symv"] = symts
    ...
    if session['step'] == "END":
        if result is not None:
            if result[0] != session["testpred"]:
                session['step'] = "Q_C"
                return "As you provide me with few symptoms, I cannot predict your " \
                       "disease for the moment!!! <br> Can you specify more about what you are feeling or Tap q to " \
                       "stop the conversation "
            session['step'] = "Description"
            session["disease"] = result[0]
            return "Well <b>" + session["name"] + "</b>, you may have <b>" + result[
                0] + "</b>. Tap <b>D</b> to get a description of the disease ."
        else:
            session['step'] = "Q_C"  # test if user want to continue the conversation or not
            return "can you specify more what you feel or Type <b>quit</b> to stop the conversation"
    if session['step'] == "Description":
        ...
        if session["disease"] in description_list.keys():
            return "<b>" + description_list[session["disease"]] + "</b> \n <br> From how many days have you had symptoms?"
        else:
    ...
    if session['step'] == "Severity":
        ...
        if calc_condition(session["all"], int(s)) == 1:
            ans = specialist(session["disease"])
            doc = doctor(session["disease"])
            msg = "For your disease you should consult a Doctor who is a "
            m = msg + ans + '\n'
            flag = "<br>We found a doctor "
            flag = flag + "<b>" + doc + "</b>"
            h = flag + " Who is the best doctor for your disease "
            gg = m + h
            return gg
            ...
            for e in precautionDictionary[session["disease"]]:
                msg += '\n ' + str(i) + ' - ' + e + '<br>'
                i += 1
            msg += ' Enter <b>Quit</b> to end'
            return msg
    if session['step'] == "FINAL":
        session['step'] = "BYE"
        return "Your diagnosis was perfectly completed. Do you need another medical consultation <b>(yes or no)</b>? "
    if session['step'] == "BYE":
        name = session["name"]
        age = session["age"]
        gender = session["gender"]
        ...
        if s.lower() == "yes":
            ...
            session["age"] = age
            session['step'] = "FS"
            return "HELLO again <b>" + name + "</b> Please tell me your main symptom. "
        else:
            return "THANKS " + name + " for using <b>DiagnoBotIQ</b><br> For more information please contact<br><b>RAM KUMAR</b><br> <b> +917997884172</b>"
